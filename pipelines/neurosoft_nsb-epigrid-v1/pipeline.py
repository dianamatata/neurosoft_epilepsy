...
def _load_recordings_tolerant(raw_dir, recording_ids, modality):
    """Same as ``auditorydecoding.data.neurosoft_pipeline.load_recordings``, except it keeps a recording's own ``meas_date`` instead of raising`KeyError`` when its sidecar is missing the non-standard``OriginalRecordingTimestamp`` field.
    Some nsb-epigrid-v1 recordings (e.g. Comparator/Test acquisitions) were curated without that field.
    """
    session = {}
